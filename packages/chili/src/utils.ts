...
export function placeNodeAvoidingOverlap(document: IDocument, node: VisualNode) {
    ...
    let dx = 0, dy = 0, leg = 0, run = 0, limit = 256;
    while (limit-- > 0) {
        switch (leg & 3) { case 0: dx += step; break; case 1: dy += step; break; case 2: dx -= step; break; default: dy -= step; }
        run++;
        if (run === ((leg >> 1) + 1)) { leg++; run = 0; }
